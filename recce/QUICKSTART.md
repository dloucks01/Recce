...
| **Deep pass (unauth)** | `./bin/recce sweep -o eng` | **all** credential-free deep modules at once (web/smb/ftp/ldap/snmp/mongodb/redis/elasticsearch/docker/k8s/mssql) — skips services you don't have |
...
> `redis`, `elasticsearch`, `docker`, `k8s`, `mssql` (see the **Runbook** tab in
> the workbook).
